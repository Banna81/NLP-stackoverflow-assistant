import random
import rasa_nlu
from bs4 import BeautifulSoup
from rasa_nlu.training_data import load_data
from rasa_nlu.config import RasaNLUModelConfig
from rasa_nlu.model import Trainer
from rasa_nlu import config

import requests
import urllib, re
from urllib.parse import urljoin
import urllib, re
#from rasa_nlu.featurizers.count_vectors_featurizer import CountVectorsFeaturizer
from urllib.parse import urljoin
from urllib.request import urlopen
from rasa_nlu import config
import json
import spacy
from rasa.nlu.classifiers.classifier import IntentClassifier


name = "StackOverflow Assistant"
weather = "cloudy"
pipeline = [
    "nlp_spacy",
    "intent_featurizer_spacy",
    "intent_classifier_sklearn",
    "intent_featurizer_ngrams"
]
pipeline2 = [
    "tokenizer_whitespace",
    "ner_crf",
    "ner_synonyms",
    "intent_featurizer_count_vectors",
    "intent_classifier_tensorflow_embedding"
]
# Define a dictionary containing a list of responses for each message


def responseSearch(intend):
    mylines = []  # Declare an empty list
    randomResponses = []
    intend = "## intent:{0}\n".format(intend)
    #print(intend)
    with open('responses.md', 'rt') as myfile:  # Open file lorem.txt for reading text
        for line in myfile:  # For each line of text,

            mylines.append(line)  # add that line to the list.
        for element in mylines:  # For each element in the list,
            if element == intend:
                #print(element)
                flag = True
            if element.startswith('##') and element != intend:
                flag = False
            if element.startswith('-') and flag == True:
                #print(element)
                randomResponses.append(element)
    return randomResponses
    # print it, without extra newlines.
    #print(randomResponses)


# Use random.choice() to choose a matching response
def respond(message, intend):
    # Check if the message is in the responses
    randomm = []
    if(intend=="search"):
        question=get_question(message)
        print(question)
        params = urllib.parse.urlencode({'q': question})
        url = ("http://stackoverflow.com/search?%s" % params)
        linkslist=links(url)
        print(len(linkslist))
        if(len(linkslist)==0):
            bot_message="Sorry, I didn't find any answer for your question on StackOver flow."
        else:
            bot_message="This is the most voted answer : {0}".format(linkslist[0])
    else:
        randomm = responseSearch(intend)
        bot_message = random.choice(randomm)
        bot_message = re.sub('- ','',bot_message)
    return bot_message


# Define a function that sends a message to the bot: send_message
def send_message(message, intend):
    # print(message)
    response = respond(message, intend)
    #print(response)

def links(url):
    linkslist=[]
    html = requests.get(url).content
    soup = BeautifulSoup(html,"html.parser")
    for div in soup.find_all("div", {"class": "result-link"}):
        for link in div.select("a.question-hyperlink"):
            #print ("https://stackoverflow.com/{0}".format(link['href']))
            linkslist.append("https://stackoverflow.com/{0}".format(link['href']))

    return linkslist


def get_question(randstr):
    import re
    randstr=randstr.lower()
    # Substituting multiple spaces with single space
    randstr=re.sub(r'\s+', ' ', randstr, flags=re.I)

    question_word_list = ['what', 'where', 'when', 'how', 'why', 'did', 'do', 'does', 'have', 'has', 'am', 'is', 'are',
                          'can', 'could', 'may', 'would', 'will', 'should'
                                                                  "didn't", "doesn't", "haven't", "isn't", "aren't",
                          "can't", "couldn't", "wouldn't", "won't", "shouldn't", "about"]
    for str in question_word_list:
        result = re.search(str, randstr)
        if result != None:
            quest = str
            break

    span = re.search(quest, randstr).span()
    index = span[0]
    all_sen = randstr[index:]
    question=all_sen
    if re.search(r'[?]', all_sen) != None:
        mark_span = re.search(r'[?]', all_sen).span()
        mark_index = mark_span[0]
        question = all_sen[:mark_index]
    else:
        mark_index = -1

    return question


args = {'pipeline': pipeline2}
trainer = Trainer(config.load("tensor.yml"))
# Load the training data
training_data = load_data("intentt.md")
# Create an interpreter by training the model
interpreter = trainer.train(training_data)




# print(intent["name"])
# responseSearch(intent["name"])
# for ent in entities:
# params[ent["intent"]] = str(ent["value"])
# print(params)

#print(intent)
#print(intent["name"])



#message = input()
#while (message != 'exit'):


    #message = preprocessing(message)

    #intent = interpreter.parse(message)["intent"]
    #rank = interpreter.parse(message)
    #print(rank)
   # send_message(message, intent["name"])
   # message = input()
